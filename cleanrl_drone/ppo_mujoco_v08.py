...
class Args:
    ...
    env_id: str = "custom_envs/TacDroneHover-v5"
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if action is None:
            ...
            action = torch.clamp(action, -1.0 + TANH_EPS, 1.0 - TANH_EPS)
            raw_action = torch.atanh(action)

        logprob = probs.log_prob(raw_action)
        logprob -= torch.log(1.0 - action.pow(2) + TANH_EPS)
        logprob = logprob.sum(1)
        return action, logprob, probs.entropy().sum(1), self.critic(x)

    def get_deterministic_action(self, x):
        return torch.tanh(self.actor_mean(x))
